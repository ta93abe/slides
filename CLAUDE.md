...
├── .scaffdog/slide.md             # 新規スライドのテンプレート
...
- `slides/`配下のディレクトリを走査し、`package.json`に`slidev`フィールドがあるものをスライドとして自動検出
...
- **Slidev** (v51+): Markdownベースのプレゼンテーションフレームワーク（Vue 3）
- **pnpm**: パッケージマネージャ（ワークスペース管理）
- **scaffdog**: スライドプロジェクトのスキャフォールディング
