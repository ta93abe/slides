...
データパイプラインは長時間実行されます。その間に何が起きるでしょうか。
...
- **重複実行** — 同じ step が 2 回走ると副作用が重複します
...
- 「どこまで進んだか」の **永続化**
- クラッシュ後に **途中から再開**
- 各 step は **at-least-once** で実行されます
- 長期待機中は **計算リソースを使いません**

<div class="mt-4 text-sm opacity-80">
素朴に自前で書くと、状態テーブル・冪等キー・リトライキュー・タイマー ...<br>
実質「分散システムの難問」の再発明になります。
</div>
...
基本アイデア: **各 step の結果を自動永続化し、クラッシュ後はリプレイで state を復元します。**
...
- **Hibernation** — `step.sleep()` / `step.waitForEvent()` 中はプロセス停止、**課金されません**（最大 365 日）
...
先に見た DO の <strong>グローバル一意 ID + 永続ストレージ</strong> が、Workflows の実行モデルをそのまま支えています。
</div>
...
- step 内の副作用は **冪等** に書きます（at-least-once = 重複実行される可能性あり）
...
Temporal / Step Functions に相当します。TypeScript でコードとして定義できます。
...
**vs Step Functions**: Step Functions は ASL（JSON）でフローを定義します。Workflows は TypeScript でロジックとフローが一体です。Step Functions の方がビジュアルエディタ・実行履歴UIが成熟しています。Workflows はコードファーストで軽量ですが、GUI での可視化は弱めです。
